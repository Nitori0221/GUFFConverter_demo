...
```
Dockerfile.lab-gpu          # CUDA12.4/Ubuntu22.04, llama.cpp を CUDA ビルド、Jupyter 付き
compose.yaml                # GPU パススルー & /workspace マウント、Jupyter の起動設定
.env.example                # 環境変数サンプル（JUPYTER_TOKEN 等）
notebooks/GGUF_Convert_Quantize.ipynb  # 変換 & 量子化ノート（進捗バー・静かなログ表示）
work/                       # ホスト側の作業ディレクトリ（/workspace にマウント）
README.md                   # このファイル
```

作業ディレクトリのマウント：`./work`（ホスト） ←→ `/workspace`（コンテナ）  
...
rm -rf work/hf/.cache/hub/models--sbintuitions--sarashina2.2-3b-instruct-v0.1
```
...
- 既定：`CUDA_ARCHS=75`（T4）
- 例：RTX 30 系は `86`、A100 は `80` など
```bash
docker compose build --build-arg CUDA_ARCHS=86
```
